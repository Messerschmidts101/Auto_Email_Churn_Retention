...
class utils:
    def __init__(self):
            ...
            'DATAROBOT_API_KEY':None,
            'DATAROBOT_ENDPOINT':None,
            'DR_Project_Name':'Churn Prediction Latvia BMW & JLR',
            ...
            self.objDR_Client = None # to be filled later when triggering datarobot functions
    ...
    # tested and vetted
    def auth_datarobot(self, dicParams = None):
        strDR_API_KEY = self.get_parameter_value(dicParams = dicParams, strKey = 'DATAROBOT_API_KEY')
        strDR_ENDPOINT = self.get_parameter_value(dicParams = dicParams, strKey = 'DATAROBOT_ENDPOINT')
        self.objDR_Client = dr.Client(token=strDR_API_KEY, endpoint=strDR_ENDPOINT)
    ...
    def get_prediction_report(self, tblInput, dicParams = None, boolTest = False, boolVerbose = None):
        ...
        """    
        """ 
        # only works if project is deployed
        objJob, tblResult = dr.BatchPredictionJob.score_pandas(
            df=tblInput,
            deployment=strDR_DEPLOYMENT_ID,
            max_explanations=5,
            include_probabilities=False,
            include_probabilities_classes=["1"],
            # passthrough_columns_set="all",
            skip_drift_tracking=False
        )
        objJob.wait_for_completion(max_wait=1200)
        strJobStatus = objJob.get_status()['status']"""
        strColumnName_new_Churn_Flag = self.get_parameter_value(dicParams = dicParams, strKey = 'new_Churn_Flag')
        strColumnName_new_SERVD_Start = self.get_parameter_value(dicParams = dicParams, strKey = 'new_SERVD_Start')
    ...
    # slightly unfinished: what to do next after training the mdoel??? is this really necessary???
    def train_model(self, tblInput, dicParams = None, boolVerbose = False):
        strColumnName_new_Churn_Flag = self.get_parameter_value(dicParams = dicParams, strKey = 'new_Churn_Flag')
        strDR_Project_Name = self.get_parameter_value(dicParams = dicParams, strKey = 'DR_Project_Name')
        self.auth_datarobot(dicParams=dicParams)
        tblMaster = tblInput.toPandas()
        ##################################################
        #####                                        #####
        #####      Step 1: Upload Dataset To DR      #####
        #####                                        #####
        ##################################################
        tblMaster = dr.Dataset.create_from_dataframe(tblMaster, target=strColumnName_new_Churn_Flag)
        ##################################################
        #####                                        #####
        #####         Step 2: Start Auto ML          #####
        #####                                        #####
        ##################################################
        # Create Project
        DRProject = dr.Project.create_from_dataset(tblMaster.id, project_name=strDR_Project_Name)
        # Set Target
        DRProject.set_target(target=strColumnName_new_Churn_Flag, mode=dr.AUTOPILOT_MODE.FULL_AUTO)
        # Run
        DRProject.start_autopilot()

    ##################################################
    #####                                        #####
    #####           [Outdated Functions]         #####
    #####     These are functions for that are   #####
    #####       outdated; used by other non      #####
